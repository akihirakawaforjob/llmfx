...
class EntryConfig:
    ...
    ダウ理論の言葉でのレンジ = 「高値が切り上がり安値も切り上がる(UP)」でも
    「高値が切り下がり安値も切り下がる(DOWN)」でもない状態。
    `DowAnalyzer.structure_trend()` がそのまま判定を返す。

    レンジの中では、抵抗を上抜けたように見えても帯の中へ戻される
    (ダマシ)。裁量では「レンジは触らない、触るなら折り返しで」が
    定石で、実測でも符号が割れた(11 銘柄 407 件、開発用):

        全部            407 件  +0.115 R
        構造=レンジ のみ  153 件  -0.211 R
        構造=レンジ を除く 254 件  +0.312 R

    判定は確定済みスイングだけを使うので先読みにならない。
    """
    """損切り根拠の起点。
